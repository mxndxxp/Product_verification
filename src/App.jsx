...
const WEBSITE_URL = "http://localhost:5173"; // change to deployed URL when live

/* ===================== PRODUCTS FOR QR ===================== */
const PRODUCTS_FOR_QR = [
          ...
          {PRODUCTS_FOR_QR.map((p) => {
            ...
            return (
              <div key={p.token} className="bg-white rounded-xl p-4 shadow flex flex-col items-center text-center">
                ...
                <p className="text-[10px] text-gray-400 mt-1 break-all">{qrUrl}</p>
              </div>
            );
          })}
        </div>
        ...
        <div className="bg-gradient-to-br w-full md:w-80 from-[#0087CC] to-[#0066AA] rounded-t-[1.5rem] md:rounded-t-[2rem] rounded-b-[1rem] px-2 md:px-4 pb-2 shadow-2xl">
          ...
          <div className="bg-white rounded-xl overflow-hidden">
            ...
            <div className="px-3 md:px-6 pb-4 md:pb-4 -mt-6 md:-mt-12">
              ...
              {status === "fail" && (
                <div className="text-center py-4 min-h-[300px] border md:py-6 px-1 md:px-4 animate-in fade-in rounded-xl bg-gray-200 duration-500">
                  <h2 className="text-[11px] md:text-xl font-bold font-['Times_New_Roman'] text-red-500">WARNING!</h2>
                  <div className="space-y-4">
                    <p className="text-left font-semibold font-['Times_New_Roman'] text-red-500 mt-2 px-0 md:px-1 text-[9px] md:text-[15px] leading-snug">Your Product could not be Authenticated!</p>
                    <p className="text-left font-semibold font-['Times_New_Roman'] text-red-500 px-0 md:px-1 text-[9px] md:text-[15px] leading-snug">Do not Purchase this Product or return it to the Pharmacy!</p>
                    <p className="text-left font-semibold font-['Times_New_Roman'] text-red-500 px-0 md:px-1 text-[9px] md:text-[15px] leading-snug">Counterfeited medicines are potentially lethal!</p>
                  </div>
                  <button onClick={isQRMode ? resetQR : resetForm} className="mt-4 px-4 py-2 text-[9px] md:text-sm bg-rose-600 text-white rounded-lg font-bold hover:bg-rose-700 transition-all">
                    Try Again
                  </button>
                </div>
              )}
